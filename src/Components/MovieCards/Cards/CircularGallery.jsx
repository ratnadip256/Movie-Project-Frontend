...
class Title {
  constructor({ gl, plane, renderer, text, textColor = '#545050', font = '40px sans-serif' }) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
export default function CircularGallery({ items, ...rest }) {
  const containerRef = useRef(null);

  useEffect(() => {
    const app = new App(containerRef.current, { items, ...rest });
    return () => app.destroy();
  }, [items]);
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full overflow-hidden cursor-grab active:cursor-grabbing pointer-events-auto relative z-0"
    />
  );
}
